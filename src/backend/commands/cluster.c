...
#if 0 /* TODO: NEED TO PROCESS FULL VACUUM  */
#ifdef SMARTSSD_DBUF
	Oid relid = OldHeap->rd_id;
	uint32 max_segno = 
		((table_relation_size(OldHeap, MAIN_FORKNUM) + (BLCKSZ - 1)) / 
		BLCKSZ + (RELSEG_SIZE - 1)) /  RELSEG_SIZE;
#endif
#endif
...
#if 0 /* TODO: NEED TO PROCESS FULL VACUUM  */
#ifdef SMARTSSD_DBUF
	if (OIDNewHeap != relid && IsNormalObject(relid))
	{
#ifdef SMARTSSD_DBUF_DEBUG
		fprintf(stderr, "%u %u\n", relid, OIDNewHeap);
		fprintf(stderr, "Full vacuum executed\n");
#endif
		ndpsmgrreleasedirtybufid(relid, max_segno, true);
	}
#endif
#endif
	/*
